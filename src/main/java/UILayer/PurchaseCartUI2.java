...
public class PurchaseCartUI2 extends VerticalLayout {
    ...
    private void refreshCart() {
        ...
        for (ShoppingBag bag : userConn.getShoppingBags(token)) {
            /* capture bag-level storeId once */
            final String sid = bag.getStoreId();

            for (Map.Entry<String, Integer> e : bag.getProducts().entrySet()) {
                final String pid  = e.getKey();     // snapshot productId
                int          qty  = e.getValue();

                if (qty <= 0) continue;             // hide zero-qty rows

                String productName;
                try {
                    productName = productRepo.getById(pid).getName();
                } catch (Exception ex) {
                    /* product deleted since it was put in the cart – clean up */
                    userConn.removeFromCart(token, sid, pid, qty);
                    cartChanged = true;
                    continue;
                }
                ...
                Button rm = new Button("Remove 1", click -> {
                    try {
                        userConn.removeFromCart(token, sid, pid, 1);
                        refreshCart();               // redraw
                    } catch (Exception ex) {
                        error.setText(ex.getMessage());
                    }
        ...
        if (cartChanged)
            Notification.show("Cart automatically updated (product no longer exists)");
    }
