...
class DiscountPolicyTest {
    ...
    void init() {
        ...
        Discount productDisc = new Discount(store.getId(), 1, 0, 2, List.of(), .10f, "Apple", 0, 0, "");
        productDisc.setId(productDiscId);
        when(discountRepo.getById(productDiscId)).thenReturn(productDisc);

        Discount inner10 = new Discount(store.getId(), 1, 0, 0, List.of(), .10f, "", 0, 0, "");
        inner10.setId(innerCatDiscId1);
        Discount inner20 = new Discount(store.getId(), 1, 0, 0, List.of(), .20f, "", 0, 0, "");
        ...
        Discount catDisc = new Discount(store.getId(), 2, 0, 1, List.of(innerCatDiscId1, innerCatDiscId2), .05f, "Food", 0, 0, "");
        catDisc.setId(catDiscId);
        when(discountRepo.getById(catDiscId)).thenReturn(catDisc);

        Discount storeDisc = new Discount(store.getId(), 3, 0, 2, List.of(), .15f, "", 1, 50, "");
    ...
    void calculatePrice_noDiscounts_returnsFullPrice() {
        ...
        float total = service.calculatePrice(store.getId(), Map.of(p1.getId(), 1, p2.getId(), 1));
    ...
    void calculatePrice_productLevelMultiplication_appliesOnlyToThatProduct() {
        ...
        float total = service.calculatePrice(store.getId(), Map.of(p1.getId(), 2, p2.getId(), 1));
    ...
    void calculatePrice_storeWideMinPriceCondition_onlyIfThresholdMet() {
        ...
        float below = service.calculatePrice(store.getId(), Map.of(p1.getId(), 1));
        assertEquals(15f, below);

        float above = service.calculatePrice(store.getId(), Map.of(p1.getId(), 2, p2.getId(), 1));
