...
class RateTest {
    ...
    @Test
    void rateStore_storeNotExist_throws() {
    ...
    @Test
    void rateStore_userNotExist_throws() {
    ...
    void rateStore_rateFalse_returnsFalse() throws Exception {
        ...
        when(storeRepository.getById(STORE_ID)).thenReturn(store);
        when(userRepository.getById(USER)).thenReturn(new RegisteredUser());
        when(store.rate(3)).thenReturn(false);

        boolean result = rateService.rateStore(TOKEN, STORE_ID, 3);
    ...
    void rateProduct_productNotExist_throws() {
        when(productRepository.getById(PRODUCT_ID)).thenReturn(null);
