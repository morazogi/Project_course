...
import DomainLayer.Roles.RegisteredUser;          // <-- replace if your class lives elsewhere
...
class OpenStoreTest {
    ...
    void openStore_validToken_createsStoreAndReturnsId() throws Exception {
        ...
        when(userRepo.getById(username))
                .thenReturn(mock(RegisteredUser.class));   // any non-null user
        ...
        assertEquals(username,       saved.getFounder());  // << Store API uses ‘founder’
    ...
    @Test
    void openStore_userMissing_throwsIllegalArgument() throws Exception {
